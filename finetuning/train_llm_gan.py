...
def get_args_parser():
    ...
    parser.add_argument('--domain_num', default=25, type=int)
    ...
    parser.add_argument('--action_scheduler', default=True, type=bool)
    parser.add_argument('--domain_scheduler', default=True, type=bool)
    parser.add_argument('--init_action_epochs', default=12, type=int)
    parser.add_argument('--init_domain_epochs', default=24, type=int)
    parser.add_argument('--action_epoch', default=4, type=int)
    parser.add_argument('--domain_epoch', default=2, type=int)
    parser.add_argument('--weight_deacy', default=0.0, type=float)
    parser.add_argument('--label_smooth_rate', default=0.05, type=float)
    parser.add_argument('--domain_alpha', default=0.1, type=float)
    parser.add_argument('--conf_beta', default=0.1, type=int)
...
def train_model(model, train_data, start_epoch, epochs, optimizer: dict, scheduler: dict, 
    ...
    for epoch in range(start_epoch, epochs):
        ...
        model.train()
        model.to(device)
        epoch_temp = (epoch-args.init_action_epochs-args.init_domain_epochs)%(args.domain_epoch+args.action_epoch)
        ...
        for i,(inputs, action_labels, domain_labels) in bar:
            ...
            if epoch<args.init_action_epochs:
                train_action = True
            ...
            elif epoch<args.init_domain_epochs:
                train_action = False
                domain_loss = cls_loss(domain_logits, domain_labels)
                ...
                if epoch_temp<args.action_epochs:
                    train_action = True
                    action_loss = cls_loss(action_logits, action_labels)
                    domain_loss = cls_loss(domain_logits, domain_labels)
                    conf_loss = confidence_loss(action_logits)
        ...
        if eval and epoch>=args.init_domain_epochs:
...
def eval_model(model, eval_data, device, args):
    ...
    with torch.no_grad():
        ...
        print(f"The Accuracy score of model is:{eval_acc:.4f}")
        if eval_acc > global_eval_acc:
            global_eval_acc = eval_acc
            torch.save(model.state_dict(), os.path.join(args.output_dir, f'LLM_GAN_{eval_acc:.4f}.pth'))
...
def main():
    ...
    action_optimizer = optim.Adam(
        gan_model.feature_extracter.parameters(),
        lr=args.action_lr, 
    ...
    # 5. Scheduler
    s_epoch = args.epoch-args.init_domain_epochs-args.init_action_epochs
    l_epoch = args.action_epoch+args.domain_epoch
    scheduler = {}
    if args.action_scheduler:
        action_scheduler = optim.lr_scheduler.CosineAnnealingLR(action_optimizer, T_max= args.init_action_epochs+(s_epoch*args.action_epoch)//l_epoch, eta_min=5e-6)
        scheduler['action'] = action_scheduler
    if args.domain_scheduler:
        domain_scheduler = optim.lr_scheduler.CosineAnnealingLR(domain_optimizer, T_max= args.init_domain_epochs+(s_epoch*args.domain_epoch)//l_epoch, eta_min=5e-6)
