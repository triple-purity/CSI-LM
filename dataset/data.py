...
def get_bvp_data(data_path: str, req_domains: List[str], test_size = 0.2):
    ...
    # file_name user--gesture_type--torso_location--face_orientation--repeat_num
    file_names = os.listdir(data_path)
    pro_file_names = ['-'.join(name.split('-')[:6]) for name in file_names]
    file_names = np.array(list(file_names))
    
    domains_res = {d:[] for d in req_domains}
    for name in pro_file_names:
        atris_dict = split_file_name(name)
        for d in req_domains:
            domains_res[d].append(atris_dict[d])
